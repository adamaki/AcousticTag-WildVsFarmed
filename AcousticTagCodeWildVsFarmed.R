...
dayfile = "run_1LLF15S100159_day_coded.csv" # change to file to be analysed
...
selected <- rbind(selected, subset(dayfile_tbl, Period == fishid_tbl[i,'Period']))  
}
dayfile_tbl <- selected 
rm(selected)
...
hide8SW <- subset(dayfile_tbl, Period == hidetag8SW)
names(hide8SW)[names(hide8SW) == "EchoTime"] <- "HideTime"
hide8NE <- subset(dayfile_tbl, Period == hidetag8NE)
names(hide8NE)[names(hide8NE) == "EchoTime"] <- "HideTime"


dayfile_tbl <- subset(dayfile_tbl, !(Period == hidetag8SW | Period == hidetag8NE))
...
names(dayfile_tbl)[names(dayfile_tbl) == c('HID1.x', 'HID1.y', 'HID1.z', 'HID2.x', 'HID2.y', 'HID2.z')] <- c('P8SW.x', 'P8SW.y', 'P8SW.z', 'P8NE.x', 'P8NE.y', 'P8NE.z')
...
all <- data.frame(ts = unique(unlist(c(dayfile_tbl$EchoTime, probe.DOT8$DO.time.8m))))
...
dayfile_tbl$O8 <- all$DO.8m
dayfile_tbl$T8 <- all$Temp.8m

all <- data.frame(ts = unique(unlist(c(dayfile_tbl$EchoTime, probe.sal8$Sal.time.8m))))
